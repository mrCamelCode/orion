...
import { decodePacket, encodePacket } from '../network.util.ts';

describe('Network Util', () => {
  describe('encodeWsMessage', () => {
    test('puts info in format of method:payload', () => {
      assert(encodePacket(ServerWsMethod.ClientRegistered, { token: '123' }).split(':').length === 2);
    });
    test('the payload is base64-encoded', () => {
      assertEquals(
        encodePacket(ServerWsMethod.ClientRegistered, { token: '123' }),
